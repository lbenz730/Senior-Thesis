...
x <- read.csv("NCAA_Hoops_Results_2017_Final.csv", as.is = T)
x <- mutate(x, "pred_score_diff" = predscorediff,
       "date" = as.Date(paste(year, month, day, sep = "-"))) %>%
  select(team, opponent, date, pred_score_diff) %>% rbind(
    select(read.csv("training.csv", as.is = T),
           team, opponent, date, pred_score_diff)
    )

files <- unlist(sapply(dir("pbp_data", full.names = T), dir, full.names = T))
n <- length(files)
